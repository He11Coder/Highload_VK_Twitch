...
Со стороны стримера прокси-сервер принимает поток видеосигнала по протоколу RTMP, находит необходимый сервер (```Origin```), на котором видео будет транскодировано в формат HLS. Поскольку данная задача является довольно затратной по ресурсам, было принято решение выделить для нее отдельные машины. С Origin видео выходит в готовом к передаче по HLS формате и отправляется в CDN. Дополнительно Origin'ы имеют возможность писать данные в S3-хранилище и вносить полезную информацию о трансляциях в PostgreSQL.

### 6. Чат

![](pics/6_component.png)

Как уже было описано, чат в Twitch является одной из самых восоконагруженных частей приложения. В целях оптимизации походов в базу мы пишем сообщения в Aerospike в асинхронном режиме. Взаимодействие с этим модулем происходит по протоколу WebSocket напрямую с клиентом.

### Сбор метрик

Как можно легко убедиться, модуль 7, а также все модули вида x.1 отвечают за сбор продуктовых и техничесих метрик с реализуемых сервисов. В качестве базы для хранения записей с метриками был выбран VictoriaMetrics.
...
|CDN            |Резервирование      |Необходимо создать распределенную систему CDN-серверов для опеспечения достаточного уровня резервирования. В случае, если один из серверов выходит из строя или оказывается перегружен, Origin начинает раздавать трансляцию на другие машины, а пользователи при помощи Retry-стратегии переподключаются к ним               |
...
* **Использование архитектурных паттернов**
  * **Грамотно выстроенные Retry-стратегии:** при возниковении ошибки при обращении к сервису, можно попытаться выполнить запрос еще раз в надежде все-таки получить ответ. Ранее уже был приведен пример использования Retry-стратегии для переподключения зрителя к новому CDN-серверу. Важно отметить, что число таких повторных запросов должно быть каким-то образом ограничено, чтобы не завалить и без того вероятно загруженный сервис однотипными запросами. Это может быть реализовано при помощи следующего паттерна
