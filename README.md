...
Почти все таблицы (USER, STREAM_RECORD, STREAM_RECORD_INFO, SUBSCRIPTION, VIEW, CHAT_MESSAGE), зависящие от популярности автора можно вынести в отдельын шарды, опираясь на число подписчиков, просмотров того или иного автора, так как эти таблицы для них будут обновляться и читаться наиболее часто. Остальных пользователей можно распределять по шардам, используя хэш от ```id```.
