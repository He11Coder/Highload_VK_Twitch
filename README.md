...
* [**10. Обеспечение надёжности**](#10-обеспечение-надёжности)
...
$$ 16\space байт\text{ (uuid - id)} + 10\space байт\text{ (text - username)} + 40\space байт\text{ (text - email)}+$$
$$+10\space байт\text{ (bytea - salt)} + 32\space байта\text{ (bytea - password)}+$$
$$+500\space байт\text{ (text - description)} + 60\space байт\text{ (text - avatar-url)} +$$
$$+ 8\space байт\text{(timestamp - created-at)}=676\space байт/пользователя $$
...
$$ 16\space байт\text{ (uuid - id)} + 16\space байт\text{(uuid - stream-record-id)} +$$
$$+ 50\space байт\text{ (text - title)} + 500\space байт\text{ (text - description)} + $$
$$+ 8\space байт\text{ (timestamp - created-at)}=590\space байт/стрим $$
...
$$ 16\space байт\text{ (uuid - id)} + 16\space байт\text{ (uuid - stream-record-id)} + $$
$$+ 16\space байт\text{ (uuid - author-id)} + 400\space байт\text{ (text - message)} +$$
$$+ 8\space байт\text{ (timestamp - created-at)}=456\space байт/сообщение $$
...
$$ 16\space байт\text{ (uuid - viewer-id)} + 16\space байт\text{ (uuid - stream-record-id)} + $$
$$+8\space байт\text{ (timestamp - created-at)} + 8\space байт\text{ (timestamp - updated-at)}=48\space байт/запись $$
...
$$ 16\space байт\text{ (uuid - subscriber-id)} + 16\space байт\text{ (uuid - author-id)} + $$
$$+8\space байт\text{ (timestamp - created-at)}=40\space байт/запись $$
...
$$ 16\space байт\text{ (uuid - user-id)} + 16\space байт\text{ (uuid - session-id)} +$$
$$+ 8\space байт\text{ (timestamp - created-at)}=40\space байт/сессию $$
...
### Transmuxing/Transcoding [[10](https://blog.twitch.tv/en/2017/10/10/live-video-transmuxing-transcoding-f-fmpeg-vs-twitch-transcoder-part-i-489c1c125f28/)]
...
|------------------------|-------------------|-------------------|
|Go                      |Backend            |Популярный современный язык для написания backend-части приложений. Быстрый, компилируемый язык, по природе хорошо подходит для работы под высокими нагрузками. Также хорошо подходит для микросервисной архитектуры (gRPC, ProtoBuf и т.д.), известен своей асинхронной моделью, что обеспечит и упростит асинхронную обработку запросов. Дополнительным плюсом является наличие большого количества библиотек, пакетов и драйверов, реализующих самый разный функционал и упрощающих написание кода  |
...
|Kotlin                  |Mobile, Android    |Кроссплатформенный, популярный язык с большим количеством технологий и специалистов             |
|Swift                   |Mobile, iOS        |Популярный язык с большим количеством технологий   |
|Nginx                   |Proxy/балансировка нагрузки            |Надежный, популярный, поддерживаемый инструмент для реализации различных сценариев балансировки нагрузки, терминации SSL-соединений, обработки HTTP(S) запросов, возможность конфигурации под используемые в нашем сервисе протоколы HLS и RTMP                     |
|HLS                     |Стриминг, раздача контента пользователям|Широко используемый протокол для видеостриминга по HTTP от Apple. Удобен встроенной возможностью стриминга с технологией ABR Streaming (стриминг с адаптивным битрейтом) и передачи субтитров, а также поддерживает большое количество кодеков                  |
|RTMP                    |Стриминг, получение контента от автора                   |Некогда популярный протокол в том числе и для доставки контента. В настоящее время эту нишу занял HLS, но RTMP по-прежнему широко используется именно для получения видеоконтента от первоисточника, поскольку, в отличие от HLS, обладает минимальной задержкой (<5 секунд)       |
|WebSocket               |Взаимодействие с чатом|Протокол связи поверх TCP-соединения, наиболее часто используется именно для взаимодействия с сервером в реальном времени с постоянным соединением. Отлично подходит для реализации передачи сообщений между пользователями чата|
|PostgreSQL              |Основная БД сервиса                    |Реляционная модель хорошо подходит для хранения [выбранных сущностей](#6-физическая-схема-бд), имеется широкая поддержка, много инструментов для масштабирования, транзакционная модель       |
|PgBouncer               |Мультиплексирование подключений к PostgreSQL               |Популярное для выбранной цели решение. Позволит снизить нагрузку на базу, принимая подключения от сервисов и мультиплексируя их в меньшее количество подключений к инстансам базы                |
|Redis                   |Хранение сессий    |Быстрая in-memory база данных, отлично подойдет для ускорения процесса авторизации пользователей. Имеет довольно простой и лаконичный язык запросов, подходящие встроенные типы данных (key-value), возможность установки TTL для токенов авторизации   |
|Aerospike               |Чат трансляции     |Оптимизированная под быстрое чтение/запись на SSD NoSQL база данных, имеется встроенная возможность построения индексов, кэширования и реплицирования                |
|ClickHouse              |Аналитическая СУБД |Данная СУБД отлично показывает себя под нагрузками, имеется возможность сохранения и чтения аналитических данных в real-time режиме, а также имеется встроенная функция наполнения данными из потока Kafka|
|Amazon CloudFront       |CDN                |Сервис предоставляет распределенную и масштабируемую систему для раздачи контента пользователям. Имеется поддержка протокола HLS, возможность кэширования и встроенный GEO-based балансировщик            |
|Amazon S3               |Объектное хранилище|Надежное, реплицируемое и масштабируемое решение для хранения статического контента. Популярный сервис с удобным API для загрузки/получения данных    |
|Kafka                   |Брокер сообщений   |Высокопроизводительный, масштабируемый сервис для обеспечения асинхронного взаимодействия компонент архитектуры сервиса. Легко интегрируется с другими сервисами и технологиями благодаря своей популярности                 |
|VictoriaMetrics         |Хранилище метрик   |Эффективное хранилище, поддержка pull и push стратегий сбора метрик, имеет собственный язык запросов с широким функционалом, легко интегрируется с Go и предоставляется в AWS   |
|Grafana                 |Мониторинг         |Популярный инструмент, совместим с большим числом технологий, имеется возможность настройки оповещений и алертов|
|Docker                  |Контейнеризация    |Отлично подходит для работы с Kubernetes, эффективно упаковывает сервисы и использует ресурсы сервера      |
|Kubernetes              |Оркестрация        |Широко известный инструмент для оркестрации микросервисов, обеспечивает отказоустойчивость, масштабирование сервисов и эффективную утилизацию ресурсов машины за счет управления контейнерами в зависимости от нагрузки|
...
Входной точкой для клиента в нашем серверном приложении является один из инстансов ```Nginx```, балансирующего нагрузку на ```API Gateway``` и терминирующего SSL, как видно из схемы (запросы внутри системы "ходят" по HTTP). Сам API Gateway позволяет дополнительно изолировать компоненты системы и собирать метрики API, о чем подробнее будет сказано позднее.

Рассмотрим каждую из компонент в отдельности с целью описания потоков входных и выходных данных.

### 1. Авторизация

![](pics/1_component.png)

Этот модуль ответственен за работу с токенами сессий. Серверное приложение ```Authenticator``` принимает входящие запросы, создает сессии и асинхронно сохраняет их в ```Redis``` с целью ускорения данного процесса. При необходимости обращается в другие модули (например, ```User```) через API Gateway. Все взаимодействия с модулем происходят по протоколу HTTP.

### 2. Пользователь

![](pics/2_component.png)

Данный компонент контролирует базу данных ```PostgreSQL``` с информацией о пользователях, обращаясь в нее через ```PgBouncer``` с целью снижения нагрузки на базу (в смысле уменьшения числа TCP-подключений к СУБД). В этом же модуле расположено S3-хранилище с аватарками пользователей. Все записи происходят в синхронном режиме, поскольку информация о пользователях является важной и для нас недопустимо ее терять без возможности должным образом оповестить клиента. Аналогично предыдущему компоненту, основной протокол взаимодействия - HTTP.

### 3. Метрики канала

![](pics/3_component.png)

Как было [описано ранее](#6-физическая-схема-бд), мы используем Click House для сбора, хранения и раздачи информации о подписчиках того или иного канала, а также о просмотрах конкретных видео. Данная информация может быть использована как аналитическая для авторов или внутренних аналитиков компании, поэтому была выбрана соответствующая СУБД. Данные в нее попадают асинхронно через брокер сообщений Kafka. Это позволяет не ждать записи очередного просмотра или подписчика в базу и, дополнительно, Click House легко настраивается для работы с этим инструментом.

### 4. VOD-стриминг

![](pics/4_component.png)

Хранение и раздача записей трансляций (Video On Demand) является важной функциональной особенностью всего приложения. Изначально мы имеем в ```PostgreSQL``` соответствующую таблицу с информацией о стриме (в том числе ссылка на хранилище в S3). Через PgBouncer эти данные синхронно вычитываются из базы и далее контент из S3-хранилища направляется в CDN для раздачи зрителям. Этот модуль принимает HTTP-запрос на получение видеопотока и в ответ отдает HLS-фрагменты клиенту напрямую.

### 5. Сервис стриминга в live-режиме

![](pics/5_component.png)

Данная функциональность является центральной в нашем сервисе и, поэтому, логичным образом она вынесена в отдельный большой сервис. На его реализации остановимся чуть подробнее.

![](pics/Live_streaming_service.png)

Клиент со своей стороны подключается к ближайшему прокси-серверу (для примера взят Nginx), который по ряду параметров находит наиболее оптимальный CDN и возвращает с него видеопоток по протоколу HLS. Для балансировки и поиска оптимального CDN в Twitch используется дополнительное ПО ```Routing Service```. 

Со стороны стримера прокси-сервер принимает поток видеосигнала по протоколу RTMP, находит необходимый сервер (```Origin```), на котором видео будет транскодировано в формат HLS. Поскольку данная задача является довольно затратной по ресурсам (более подробно про транскодирование было рассказано в [одном](#7-алгоритмы) из предыдущих разделов), было принято решение выделить для нее отдельные машины. С Origin видео выходит в готовом к передаче по HLS формате и отправляется в CDN. Дополнительно Origin'ы имеют возможность писать данные в S3-хранилище и вносить полезную информацию о трансляциях в PostgreSQL.

### 6. Чат

![](pics/6_component.png)

Как уже было описано, чат в Twitch является одной из самых высоконагруженных частей приложения. В целях оптимизации походов в базу мы пишем сообщения в Aerospike в асинхронном режиме. Взаимодействие с этим модулем происходит по протоколу WebSocket напрямую с клиентом.

### Сбор метрик

Как можно легко убедиться, модуль 7, а также все модули вида x.1 отвечают за сбор продуктовых и технических метрик с реализуемых сервисов. В качестве базы для хранения записей с метриками был выбран VictoriaMetrics.

## 10. Обеспечение надёжности

Подразумевается, что для обеспечения надежности в работе сервиса необходимо гарантировать устойчивость к сбоям на всех возможных уровнях (как на аппаратном уровне, уровне приложений, так и на физическом).

### Компоненты системы

|Компонент      |Способ              |Обоснование         |
|---------------|--------------------|--------------------|
|Nginx          |Резервирование      |Необходимо обеспечить систему достаточно большим количеством proxy-серверов для распределения нагрузки по ним. Более того, нужно иметь некоторое резервное количество машин для обеспечения отказоустойчивости. В случае, если какие то из основных серверов выходят из строя или в сервис приходит слишком большая нагрузка, ее часть можно направить на резервные инстансы                |
|API Gateway    |Реплицирование      |API Gateway так же как и Nginx является в некотором смысле точкой входа в систему и потенциальным узким местом. Необходимо реплицировать логику работы этого компонента за счет, например, разделения API Gateway на те машины, которые обслуживают web-клиентов и mobile-клиентов. Также необходимо установить достаточное количество инстансов для равномерного распределения нагрузки между ними    |
|Хранилища      |Реплицирование      |Для снижения нагрузки на чтение из базы или S3, необходимо реализовать систему реплик по схеме Master-Slave. Это также обеспечит лучшую сохранность пользовательских данных, повысит доступность данных и распределит нагрузку между различными репликами|
|Хранилища      |Резервное копирование|Периодическое снятие бекапов существенно снизит вероятность утери пользовательских данных|
|PostgreSQL     |Мультиплексирование подключений|Установка прокси-сервера перед PostgreSQL-базой поможет поддерживать адекватное для СУБД количество соединений с сервером      |
|ClickHouse     |Использование Kafka |Использования брокера сообщений позволит записывать данные в асинхронном режиме, что повысит общую скорость работы системы и понизит вероятность потери каких либо данных, ведь в случае, если какая то из компонент откажет или перегрузится, сообщения из очереди будут обработаны позже|
|CDN            |Резервирование      |Необходимо создать распределенную систему CDN-серверов для обеспечения достаточного уровня резервирования. В случае, если один из серверов выходит из строя или оказывается перегружен, Origin начинает раздавать трансляцию на другие машины, а пользователи при помощи Retry-стратегии переподключаются к ним               |

### Сервера и сервисы

* **Резервирование ресурсов:** установка в сервера дополнительных сетевых карт, дисков, обеспечение достаточного количества CPU, RAM и т.п.

* **Логирование и сбор метрик:** сбор продуктовых и технических метрик с сервисов для внутренней аналитики продукта или логов для отслеживания ошибок и задержек при выполнении запросов. Метрики с самих машин позволят контролировать текущее состояние серверов и в случае критической ситуации оперативно принимать необходимые решения

* **Использование архитектурных паттернов**
  * **Грамотно выстроенные Retry-стратегии:** при возникновении ошибки при обращении к сервису, можно попытаться выполнить запрос еще раз в надежде все-таки получить ответ. Ранее уже был приведен пример использования Retry-стратегии для переподключения зрителя к новому CDN-серверу. Важно отметить, что число таких повторных запросов должно быть каким-то образом ограничено, чтобы не завалить и без того вероятно загруженный сервис однотипными запросами. Это может быть реализовано при помощи следующего паттерна

  * **Circuit breaker:** данный паттерн предполагает наличие механизма, контролирующего, например, число ошибок, время ответа на запросы от сервиса. В случае преодоления этой метрикой некоторого критического порога Circuit Breaker на время запрещает выполнение запросов к сервису во избежание возможных каскадных эффектов и обрушения всей системы

  * **Timeout:** в случае, если ответ от сервиса не возвращается дольше какого-то времени, то мы идем в реплицирующий или резервный сервис

* **Внутренний контроль доступа к сервисам:** во избежание инцидентов с умышленной или случайной поломкой системы сотрудниками сервиса, необходимо выстроить четкую систему разграничения доступа к сервисам, базам и т.п.

* **Graceful shutdown:** при выключении какого либо из компонентов систем необходимо убедиться, что все ресурсы освобождены, а процессы завершены. В противном случае это может привести к росту ответов со статусом 500 и даже потере некоторых пользовательских данных

* **Graceful degradation:** отказоустойчивая система должна продолжать предоставлять пользователю максимально возможный функционал даже в случае выхода из строя некоторых ее компонент. Например, при выходе из строя сервиса ```Чат```, видеотрансляция не должна прерываться, пользователи будут получать контент, пусть и в ограниченном виде. Полный отказ в обслуживании в таком случае недопустим

### Дата-центры

* **Геораспределенное резервирование:** необходимо разместить несколько датацентров в целях обеспечения отказоустойчивости приложения при выходе из строя целого ДЦ. Более того, нужно предусмотреть вероятность природных или иных катастроф, а также обесточивания региона и т.п. Для этого размещаемые датацентры должны быть удалены друг от друга на достаточное расстояние

* **Резервирование линий питания:** в случае отказа одной из линий питания сервера и системы охлаждения продолжат свою работу от резервной линии во избежание полной потери всего ДЦ

* **Резервирование систем охлаждения:** сбой в системе охлаждения датацентра неизбежно приведет к полной его потере в течение нескольких минут. Чтобы исключить подобные сценарии, нужно всегда иметь резерв

* **Установка дизельных генераторов:** наличие генераторов позволит поддерживать датацентр в рабочем состоянии в течение некоторого времени в случае полного отказа электропитания. Возможно, что этого времени будет достаточно для устранения неполадок и отказа в обслуживании удастся полностью избежать

* **Установка источников бесперебойного питания**: установка UPS в стойках датацентра позволит реализовать принцип Graceful shutdown или даст некоторое время (порядка 10-15 минут) для смены линии питания

* **Периодические учения для персонала:** проведение регулярных учений с отключением датацентров и симуляцией других аварийных ситуаций позволит подготовить сотрудников к таким сценариям и избежать утери данных, повреждения серверов и пр.

## 11. Расчёт ресурсов

### Хранилища

На основе расчетов, проведенных в [5 части работы](#5-логическая-схема-бд) относительно нагрузок на чтение и запись и необходимого объема данных в используемых хранилищах, а также [части 2](#2-расчет-нагрузки) с расчетом используемого сетевого трафика, приведем следующую таблицу:

|Хранилище    |Объем данных|Нагрузка (запись), RPS|Нагрузка (чтение), RPS|Трафик|
|-------------|------------|----------------------|----------------------|------|
|PostgreSQL   |222 Гб      |28                    |1050                  |482.8 Кбайт/c|
|Redis        |2.3 Гб      |350                   |350                   |27.3 Кбайт/c|
|Aerospike    |1.7 Тб      |1500                  |500                   |3.475 Тбайт/с|
|ClickHouse   |193.6 Гб    |400                   |700                   |47 Кбайт/с|
|Amazon S3    |131 Пбайт   |4900                  |72000                 |1 Тбайт/c|

### Сервисы

|Сервис         |RPS      |Трафик     |
|---------------|---------|-----------|
|API Gateway    |3600     |15 Мбайт/c |
|Auth           |700      |700 Кбайт/c|
|User           |700      |14 Мбайт/c |
|Channel Metrics|1100     |86 Кбайт/c |
|VOD Streaming  |200      |6 Кбайт/c  |
|Live Streaming |350      |10 Кбайт/c |
|Origin         |10000    |200 Гбайт/c|
|CDN            |125000   |1.8 Тбайт/c|
|Chat           |2000     |3.5 Тбайт/c|

*Примечание.* Показатель RPS для сервиса `API Gateway` был рассчитан следующим образом (исходя из факта, что данный сервис является шлюзом для почти всех остальных):

$$RPS_{Auth}+RPS_{User}+RPS_{ChannelMetrics}+2\cdot RPS_{VODStreaming}+$$
$$+2\cdot RPS_{LiveStreaming}=3600$$

Удвоенные значения $RPS_{VODStreaming}$ и $RPS_{LiveStreaming}$ были взяты для учета той части RPS сервиса `Chat`, которые проходят через `API Gateway` каждый раз, когда идет запрос к `VOD Streaming` (для получения записи стрима) и `Live Streaming` (для получения соединения по WebSocket).

Схожим образом рассчитан и трафик для `API Gateway`.

### Необходимые ресурсы

#### <ins>Nginx</ins>

Согласно тестам [[11](https://blog.nginx.org/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers)], сервер Nginx с 16 CPU способен держать $6\space 676$ HTTPS подключений в секунду (CPS). Такая же конфигурация способна обрабатывать $383\space  860$ RPS по HTTPS в случае, если на каждый запрос приходятся по 10 Кбайт данных. Таким образом, если принять, что $RPS_{APIGateway}=RPS_{Nginx}$, то нам будет достаточно одного сервера для обслуживания внешних HTTPS запросов.  
В [части 3](#3-глобальная-балансировка-нагрузки) были выбраны 22 дата-центра по всему миру. При равномерном распределении нагрузки на дата-центр будет приходится $\frac{3600}{22}\approx 164$ RPS, "проходящих" через Nginx. Согласно тестам [11], сервер Nginx с 2 CPU способен держать $869$ HTTPS подключений в секунду (CPS). Такая же конфигурация способна обрабатывать $48\space 654$ RPS по HTTPS в том случае, если на каждый запрос приходится по 10 Кбайт данных. Этой производительности вполне хватит даже с учетом пиковых нагрузок. Таким образом, если принять, что $RPS_{APIGateway}=RPS_{Nginx}$, то нам будет достаточно одного сервера для обслуживания внешних HTTPS запросов. Каждому дата-центру будет достаточно одного экземпляра Nginx. С учетом резервирования возьмем 2. Также 2 таких же сервера понадобиться для API Gateway в каждом ДЦ.

Имеем:

|Сервис     |CPU|Network    |Количество|
|-----------|---|-----------|----------|
|Nginx      |2  |0.5 Гбайт/c|44        |
|API Gateway|2  |0.5 Гбайт/c|44        |
...
* 1 ядро CPU = 10 RPS и 1 Мбайт RAM/запрос (тяжелая бизнес-логика - далее "Т")

Из таблицы выше получим следующую (в расчете нагрузки на **один дата-центр**):

|Сервис (сложность) |RPS                 |Трафик     |CPU|RAM, Гбайт|
|-------------------|--------------------|-----------|---|----------|
|Auth (Л)           |32                  |32  Кбайт/c|2  |2         |
|User (Л)           |32                  |700 Кбайт/c|2  |2         |
|Channel Metrics (Л)|50                  |4 Кбайт/c  |2  |2         |
|VOD Streaming (Л)  |10                  |0.3 Кбайт/c|2  |2         |
|Live Streaming (Л) |16                  |0.5 Кбайт/c|2  |2         |
|Origin (Т)         |455                 |10 Гбайт/c |46 |64        |
|CDN (С)            |5700                |82 Гбайт/c |57 |64        |
|Chat (С)           |100                 |160 Гбайт/c|2  |32        |

В общей сложности имеем:

|Сервис         |Конфигурация                          |Количество|
|---------------|--------------------------------------|----------|
...
Согласно расчетам, нагрузка на сервера PostgreSQL составит $\approx 1100$ TPS. Будем считать, что одна машина с 2 CPU и 4 Гбайта RAM выдерживает около $5\space 000$ TPS. С учетом репликации возьмем 1 Master- и 2 Slave-инстанса.

##### Redis

По официальным данным один Redis-сервер выдерживает $50\space 000$ RPS. С учетом реплик, возьмем 1 Master- и 2 Slave-инстанса в конфигурации 2 CPU + 32 Гбайта RAM + 0.5 Тбайт памяти.

##### Aerospike

Согласно официальному сайту Aerospike [[12](https://aerospike.com/blog/new-aerospike-benchmark-demonstrates-real-time-performance-at-petabyte-scale/)], одного инстанса с 4 CPU и 64 Гбайт RAM (для возможного кэширования) будет достаточно для обработки запросов от сервиса `Chat`. Также потребуется диск на 3 Тбайта (берем с запасом). Для резервирования возьмем еще одну дополнительную машину.

##### ClickHouse

Описанная в приведенной выше таблице нагрузка на чтение/запись не является существенной для ClickHouse. Памяти также много не требуется. Поэтому причине возьмем память на 2 Тбайта (запас), 2 CPU cores и 16 Гбайт RAM. Потребуется +1 машина для резервирования.

##### S3

Будем считать, что один сервер хранит 100 Тбайт данных, при этом ему нужно 24 CPU и 64 Гбайта RAM:

$$\frac{131\space Пбайт}{100\space Тбайт}=1\space 310\space машин$$

### Итого

Составим резюме для всех вышеприведенных расчетов в виде следующей сводной таблицы:

|Сервис         |CPU cores|RAM, Гбайт|Память (NVMe), Тбайт|Кол-во серверов|
|---------------|---------|----------|--------------------|---------------|
|Nginx          |2        |16        |0.5                 |44             |
